...
export function CommentItem({
  ...
  const isManagerOrOwner = ["owner", "manager"].includes(
    membership?.role || ""
  );
  ...
  const handleDelete = async () => {
    try {
      ...
      if (isReply) {
        setAllReplies((prev) =>
          prev.filter((reply) => reply._id !== comment._id)
        );
      }
    } catch (error) {
  ...
  const timestamp = formatDistanceToNow(new Date(comment.createdAt as string), {
    addSuffix: true,
  });

...
export const GET = withApi(async (request: NextRequest, { params }: { params: { taskId: string } }, decoded) => {
  ...
  const { membership } = await teamService.getTeamWithMembership(task.team.toString(), decoded.id);
  if (!membership) {
...
// Update a task
export const PUT = withApi(async (request: NextRequest, { params }: { params: { taskId: string } }, decoded) => {
  if (!decoded) {
    throw new ApiError(401, 'Unauthorized');
  }
  ...
  const { team, membership } = await teamService.getTeamWithMembership(task.team.toString(), decoded.id);
  if (!team) {
  ...
  if (result.data.assignee && result.data.assignee !== task.assignee.toString()) {
    const newAssignee = await dbService.teamMember.findById(result.data.assignee);
    if (!newAssignee || newAssignee.team.toString() !== task.team.toString()) {
  ...
  if (result.data.project && result.data.project !== task.project.toString()) {
...
// Delete a task
export const DELETE = withApi(async (request: NextRequest, { params }: { params: { taskId: string } }, decoded) => {
  if (!decoded) {
    throw new ApiError(401, 'Unauthorized');
  }
